...
: DictionaryPreferenceDialog( parent, dictionaryTypeName ), dictName( dictionaryTypeName )
{
  ...
  //Make connections
  connect( ListView, SIGNAL( added( QListWidgetItem* ) )
             , this,   SLOT( settingChanged() ) );
  connect( ListView, SIGNAL( removed( QListWidgetItem* ) ),
               this,   SLOT( settingChanged() ) );
  connect( ListView, SIGNAL( movedUp( QListWidgetItem* ) ),
               this,   SLOT( settingChanged() ) );
  connect( ListView, SIGNAL( movedDown( QListWidgetItem* ) ),
               this,   SLOT( settingChanged() ) );
...
void dictFileFieldSelector::readFromPrefs()
{
  ...
  else
  {
    ...
    config->addItem(  new KConfigSkeleton::ItemStringList(  "dicts_" + dictName
                                                          , itemName
                                                          , *new QStringList() )
                    , itemName );
    config->readConfig();
